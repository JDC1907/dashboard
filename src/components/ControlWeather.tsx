...
export default function ControlWeather() {
    ...
    const handleChange = (event: SelectChangeEvent) => {
        ...
        let idx = parseInt(event.target.value)//indice del elemento selccionado
        setSelected(idx);//en vez demostrar ahora vamos a cambiar de estado
    ...
    //aqui, si es diferente a -1 muestra la descripcion
    let items = [
    ...
    let options = items.map((item, key) => <MenuItem key={key} value={key}>{item["name"]}</MenuItem>) //renderización, antes que sea renderiza en la pantalla.
